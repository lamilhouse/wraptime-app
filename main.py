...
with st.sidebar:
    ...
    try:
        # Lectura de datos desde GSheets
        ...
        if not df_f_user.empty and not df_p_user.empty:
            ...
            # Generación del CSV con bloques
            output = io.StringIO()
            
            # Bloque superior (Datos del proyecto)
            output.write(f"REPORTE DE JORNADAS - WrapTime Lite\n")
            output.write(f"PROYECTO;{p_info['Proyecto']}\n")
            output.write(f"USUARIO;{user_id}\n")
            output.write(f"CONTRATO;{p_info['Horas_Contrato']}h dia / {p_info['Horas_Semana']}h semana\n")
            output.write("\n") # Separación
            
            # Bloque inferior (Tabla de variables)
            # index=False es CLAVE para que la semana sea la Columna A y no haya columnas vacías a la izquierda
...
# --- 2. FICHAR ---
    ...
    st.title("📝 Fichaje")
    if df_p_user.empty: st.warning("⚠️ Configura tu proyecto primero.")
    else:
