...
with st.sidebar:
    ...
    try:
        ...
        if not df_f_user.empty and not df_p_user.empty:
            ...
            # Formato fecha DD/MM/YYYY para CSV
...
# --- 3. HISTORIAL ---
    ...
    if not df_f_user.empty and not df_p_user.empty:
        ...
        for sem in sorted(df_f_user['Semana'].unique(), reverse=True):
            ...
            # Formato fecha y horas para visualización
            ...
            with st.expander(f"📂 {lbl} — {round(df_sem['Horas_Totales'].sum(), 1)}h"):
                df_tab = df_sem.rename(columns={
                    "Tipo_Dia": "Tipo", "Hora_Inicio": "Call", 
                    "Hora_Fin_Jornada": "Fin", "Horas_Totales": "H",
                    "Incidencias": "Alertas", "Observaciones": "Notas"
                })
